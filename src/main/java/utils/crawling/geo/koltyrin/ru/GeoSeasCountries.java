...
class GeoSeasCountries {
    private static final String selectSql = "select id,flag_number from dic_countries";
    private static final String selectSeaSql = "select id,country from dic_seas";
    private static final String insertSql = "insert into dic_seas_countries(sea_id,country_id) values (?,?)";
    ...
    private void parseSite() throws IOException {
        try (Connection con = DriverManager.getConnection("jdbc:sqlite:db\\solverDB.s3db");
             ...
             PreparedStatement selectSeaPS = con.prepareStatement(selectSeaSql);
             PreparedStatement insertPS = con.prepareStatement(insertSql);
             ResultSet rs = selectPS.executeQuery();
             ResultSet seaRS = selectSeaPS.executeQuery()) {
            ...
            while(seaRS.next()) {
                int seaId = seaRS.getInt("id");
                String countryFlagIds = seaRS.getString("country");
                if (countryFlagIds != null && !countryFlagIds.isEmpty()) {
                    String[] countryIds = countryFlagIds.split(",");
                    for (int i = 0; i < countryIds.length; i++) {
                        System.out.println(seaId + "    " + new Integer(countryIds[i]));
                        insert(insertPS, seaId, countryMap.get(new Integer(countryIds[i])));
                    }
                }
            }
    ...
    public static void main(String[] args) throws IOException {
        GeoSeasCountries geoSeasCountries = new GeoSeasCountries();
        geoSeasCountries.loadDBDriver();
        geoSeasCountries.parseSite();
    }
}