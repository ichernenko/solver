...
class NeoloveManName {
    ...
    private void parseSite(String nationalityFrom, String nationalityTo, int id) throws IOException {
        try (Connection con = DriverManager.getConnection("jdbc:sqlite:db\\solverDB.s3db");
            ...
            int insertsCount = 0;
            Document doc = Jsoup.connect("http://names.neolove.ru/national/male/").timeout(0).get();
    ...
    public static void main(String[] args) throws IOException {
        NeoloveManName neoloveManName = new NeoloveManName();
        neoloveManName.loadDBDriver();

        String nationalityFrom = "xxx"; // "русское";
        String nationalityTo = null; // "сабинское";

        neoloveManName.delete(nationalityFrom);
        neoloveManName.parseSite(nationalityFrom, nationalityTo, neoloveManName.getCurrentId());
    }
}